...
import de.lmu.ifi.dbs.paros.utils.PluginLoader;
import de.lmu.ifi.dbs.paros.algorithms.Algorithm;
import de.lmu.ifi.dbs.paros.clustering.Cluster;
import de.lmu.ifi.dbs.paros.clustering.ClusterTreeCellRenderer;
import de.lmu.ifi.dbs.paros.clustering.ClusterTreeModel;
import de.lmu.ifi.dbs.paros.clustering.Route;
import de.lmu.ifi.dbs.paros.clustering.SingleLinkClusteringWithPreprocessing;
import de.lmu.ifi.dbs.paros.graph.Path;
import de.lmu.ifi.dbs.utilities.Arrays2;
import de.lmu.ifi.dbs.paros.utils.OSMUtils.PATH_ATTRIBUTES;
...
import de.lmu.ifi.dbs.paros.simplex.PointPanel.PointSource;
import de.lmu.ifi.dbs.paros.simplex.SimplexControl;
import de.lmu.ifi.dbs.paros.utils.OSMUtils;
import de.lmu.ifi.dbs.paros.result.*;
...
public class TrafficminingGUI extends javax.swing.JFrame {
    ...
    private final Map<Integer, SimplexResultEntry> results = new HashMap<Integer, SimplexResultEntry>();
    private final WaypointPainter<JXMapViewer> startEndPainter = new WaypointPainter<JXMapViewer>();
    // -
    ...
    private List<TileServer> TILESERVERS = new ArrayList<TileServer>();
...
//        pbf_o.setSRTMServers(srtm);
        ...
        Map<Class, String> mapToLayoutName = new HashMap<Class, String>();
        ...
        // fill the resultTo*- maps
        Map<Class, SimplexControl> mapToControl = new HashMap<Class, SimplexControl>();
        ...
        // load recent graph
        if (properties.getFile(ParosProperties.plugin_dir) == null) {
            properties.setProperty(ParosProperties.plugin_dir, new File("./"));
            properties.save();
        }

        String lrud = properties.getProperty(ParosProperties.lru_graph_dir);
        String lruf = properties.getProperty(ParosProperties.lru_graph_file);
        boolean autoLoadGraph = properties.getBoolean(ParosProperties.autoload_graph, false);
...
//            map.setZoom(zoom);
    ...
    public void dispose() {
        ...
        properties.setProperty(ParosProperties.map_last_zoom, zoom);
        properties.setProperty(ParosProperties.map_last_center_latitude, center.getLatitude());
        properties.setProperty(ParosProperties.map_last_center_longitude, center.getLongitude());
        properties.save();
    ...
    private void loadAlgorithmComboBox() {
        File pluginDir = properties.getFile(ParosProperties.plugin_dir);
...
//                    GeoPosition coord = spf.getGeoPos();
    ...
    class LoadGraphAction implements ActionListener, PropertyChangeListener {
        ...
        public void actionPerformed(final ActionEvent evt) {
            ...
            String lruDir = properties.getProperty(ParosProperties.lru_graph_dir);
            ...
            if (JFileChooser.APPROVE_OPTION == chooser.showOpenDialog(getContentPane())) {
                ...
                properties.setProperty(ParosProperties.lru_graph_dir, lruDir);
                log.log(Level.FINE, "saving least recently used dir: {0}", lruDir);
                // setting lru file
                String lruFile = chooser.getSelectedFile().getName();
                properties.setProperty(ParosProperties.lru_graph_file, lruFile);
    ...
    private void initComponents() {
        ...
        resultTableModel = new de.lmu.ifi.dbs.paros.ReadOnlyTableModel();
        ...
        simplexControl1D = new de.lmu.ifi.dbs.paros.simplex.SimplexControl1D();
        simplexControl2D = new de.lmu.ifi.dbs.paros.simplex.SimplexControl2D();
        simplexControl3D = new de.lmu.ifi.dbs.paros.simplex.SimplexControl3D();
    ...
    private void autoloadMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_autoloadMenuItemActionPerformed
        properties.setProperty(ParosProperties.autoload_graph, autoloadMenuItem.isSelected());
        properties.save();
...
private void aboutmenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_aboutmenuItemActionPerformed
    ...
    private de.lmu.ifi.dbs.paros.simplex.SimplexControl1D simplexControl1D;
    private de.lmu.ifi.dbs.paros.simplex.SimplexControl2D simplexControl2D;
    private de.lmu.ifi.dbs.paros.simplex.SimplexControl3D simplexControl3D;
...
class ComboboxLoader {
    ...
    void load() {
        try {
            ...
            PluginLoader<Algorithm> pluginLoader = new PluginLoader<Algorithm>(pluginDir, Algorithm.class);
            List<Entry<Class<Algorithm>, File>> map = pluginLoader.getMap();
            List<AlgorithmComboBoxElement> list = new ArrayList<AlgorithmComboBoxElement>();
